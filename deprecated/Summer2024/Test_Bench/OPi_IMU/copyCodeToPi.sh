...
if [ "$1" != "" ]; then
	...
	if [ $? == 0 ]; then
		...
		ssh opi@$hostname 'rm -r /home/opi/testing_imu/'
		
		echo ">>Copying files to opi@$hostname"
		ssh opi@$hostname 'mkdir -p /home/opi/testing_imu'
		scp -r ./* opi@$hostname:/home/opi/testing_imu/
		
		echo "Compiling code."
		ssh opi@$hostname 'make -C /home/opi/testing_imu'
