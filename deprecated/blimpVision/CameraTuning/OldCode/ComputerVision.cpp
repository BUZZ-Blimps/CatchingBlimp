...
void ComputerVision::update(autoState mode, goalType goalColor){
    ...
    Mat output;
    normalize(filtered_disp, output, 0, 255, cv::NORM_MINMAX, CV_8UC1);

    namedWindow("filtered_disp");
    imshow("filtered_disp",output);
    waitKey(1);
    ...
    */
    ...
    if (mode == searching || mode == approach || mode == catching) {
        ...
        Mat matContours;
        left_correct.copyTo(matContours);
        int biggestIndex = 0;
        ...
        //drawContours(matContours,contours,biggestIndex,Scalar(255, 255, 255), -1);
        //imshow("MatContours",matContours);
