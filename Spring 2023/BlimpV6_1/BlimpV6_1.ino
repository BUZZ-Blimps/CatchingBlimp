...
double mainLoopRefresh = 10; //ms or 100Hz
...
EMAFilter vEstFilter(0.2);
EMAFilter baroFilter(0.1);
EMAFilter accFilter(1.0);
...
void setup() {
  ...
  while (millis() < 10000) {
    ...
    double alt = BerryIMU.alt;
    double altAccel = accFilter.filter((BerryIMU.AccZraw)*9.81-9.96);
    double vEst = velocityEst.update(alt, altAccel);
    Serial.println(vEst*100);
    delay(40);
  }
...
void loop() {
  ...
  if (outerLoopTime > (1.0/OUTERLOOP)*1000) {
    ...
    BerryIMU.IMU_read();
    Madgwick_F.Madgwick_Update(BerryIMU.gyr_rateXraw, BerryIMU.gyr_rateYraw, BerryIMU.gyr_rateZraw, BerryIMU.AccXraw, BerryIMU.AccYraw, BerryIMU.AccZraw);
    double pitch = Madgwick_F.roll_final;
    double roll = Madgwick_F.pitch_final;
    double yaw = Madgwick_F.yaw_final;
    double yawRate = yawRateFilter.filter(BerryIMU.gyr_rateZraw);
    double alt = baroFilter.filter(BerryIMU.alt);
    double altAccel = -BerryIMU.AccXraw*sin(pitch*3.1415/180.0)+BerryIMU.AccYraw*cos(pitch*3.1415/180.0)*sin(roll*3.1415/180.0)+BerryIMU.AccZraw*cos(pitch*3.1415/180.0)*cos(roll*3.1415/180.0);
    kf.updateBaroAccel(alt, altAccel*9.81-9.977, outerLoopTime/1000.0);

    
    Serial.println(altAccel-9.967);
    ...
    */
    ...
    if (state == manual) {
      ...
      if (manualComs.size() == 6) {
        ...
        yaw = -manualComs[0]*120;     //120 deg/s
        up = manualComs[1]*20;        //20 cm/s
    ...
    up = heightRatePID.calculate(up,vEst, outerLoopTime/1000);
    
    //update all motors
    if (millis() > 20000) {
      //motors.update(0, forward, up, yaw);
    } else {
      motors.update(0, 0, 0, 0);
    }
  }
