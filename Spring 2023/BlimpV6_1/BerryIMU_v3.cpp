...
BerryIMU_v3::BerryIMU_v3(){
  ...
  //writeTo(BM388_ADDRESS,PWR_CTRL, 0b);     // Enables pressure sensor, Enables temperature sensor, Normal mode
  //writeTo(BM388_ADDRESS,OSR, 0b);     // x16 oversampling pressure measurement, x2 oversampling temp measurement
  //writeTo(BM388_ADDRESS,ODR, 0x);     // Output data rate 25Hz
  //writeTo(BM388_ADDRESS,CONFIG, 0b);      // IIR filter coefficient of 4

  //Indoor navigation configuration (edited)
  writeTo(BM388_ADDRESS,PWR_CTRL, 0b00110011);     // Enables pressure sensor, Enables temperature sensor, Normal mode
  writeTo(BM388_ADDRESS,OSR,      0b00001101);     // x32 oversampling pressure measurement, x2 oversampling temp measurement
  writeTo(BM388_ADDRESS,ODR,      0x04);     // Output data rate 25Hz/2
  writeTo(BM388_ADDRESS,CONFIG,   0b00000111);      // IIR filter coefficient of 127
...
void BerryIMU_v3::IMU_read(){
  ...
  AccXraw = (accRaw[0] * 0.244) / 1000;
  AccYraw = (accRaw[1] * 0.244) / 1000;
  AccZraw = (accRaw[2] * 0.244) / 1000;
  ...
  gyr_rateXraw = (gyrRaw[0] * 70) / 1000;
  gyr_rateYraw = (gyrRaw[1] * 70) / 1000;
  gyr_rateZraw = (gyrRaw[2] * 70) / 1000;
