...
if [ "$1" != "" ]; then
	...
	if [ $? == 0 ]; then
		...
		ssh opi@$hostname 'rm -r /home/opi/ros2_ws/'
		
		echo ">>Copying files to opi@$hostname"
		ssh opi@$hostname 'mkdir -p /home/opi/ros2_ws'
		scp -r ./* opi@$hostname:/home/opi/ros2_ws/
		
		echo "Compiling code."
		ssh opi@$hostname 'make -C /home/opi/ros2_ws'
